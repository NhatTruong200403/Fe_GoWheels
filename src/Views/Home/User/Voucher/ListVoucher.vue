        ...
        <div class="listvoucher">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close nutthoat" @click="close()">
                ...
                <div class="modal-body">
                    <div class="add-promotion">
                        ...
                        <div v-if="promotion && promotion.length > 0 && rolePromotion == 1" style="margin-top: 15px;">
                            <div class="add-promotion__item" v-for="item in promotion" :key="item">
                                <div class="wrap-svg">hhe</div>
                                <div class="promotion-info">
                                    <p class="name">{{ item.promotion.content}}</p>
                                    <p class="detail">Giảm {{ item.promotion.discountValue*100 }} %. <span @click="ShowDetail()">Chi tiết</span></p>
                                    <div hidden style="width: 80%;">Mã khuyến mãi được tạo bởi chủ xe, và có giá trị giảm {{ item.promotion.discountValue*100 }} %. Có hiệu lực đến ngày {{ format(item.promotion.expiredDate) }}.</div>
                                </div>
                        ...
                        <div v-if="promotion && promotion.length > 0 && rolePromotion == 2" style="margin-top: 15px;">
                            <div class="add-promotion__item" v-for="item in promotion" :key="item">
                                ...
                                <div class="promotion-info">
                                    ...
                                    <p class="detail">Giảm {{ item.discountValue*100 }} %. <span @click="ShowDetail()">Chi tiết</span></p>
                                    <div hidden style="width: 80%;">Mã khuyến mãi được tạo bởi chủ xe, và có giá trị giảm {{ item.discountValue*100 }} %. Có hiệu lực đến ngày {{ format(item.expiredDate) }}.</div>
                                </div>
                                <a class="btn btn-primary" @click="UseVoucher(item.id,2)">Áp dụng</a>
                            </div>
                        </div>
                        <div v-if="!promotion && promotion.length == 0 && rolePromotion == 0">
    ...
    methods: {
        ...
        async ShowList(id) {
            if (id >= 0) {
                ...
                console.log('Post voucher', response);
